...
把输入分成“口播稿阶段”和“完整蓝图阶段”。不要因为画幅比例、视觉风格或品牌素材还没有确定，就阻塞口播稿生成。
...
工作流分为两个阶段。默认先完成口播稿确认，再展开完整蓝图；只有用户明确要求“端到端”时，才在同一轮继续，但仍必须先完成内部口播稿质量检查。
...
默认分两轮交付，避免低质量口播稿带着素材、Motion Graphics 和分镜一起扩展。
