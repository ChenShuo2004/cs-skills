...
| 将单个或一批内容想法收敛为 ChatCut 短视频 | `$xiaohuang-skill` | 选题排序、中文口播稿、素材清单、Motion Graphics、声音方向和逐镜头表 |
...
- Respect explicit skill names. `$cs-writer`, `$search-skill`, `$frontend-design`, `$clean-code`, `$checkpoint-version`, `$ralph-runner`, `$ending-time`, `$you-wendu-ip`, `$auto-videl`, and `$xiaohuang-skill` take precedence over generic routing.
...
- 将内容想法、短视频主题到 ChatCut 制作蓝图的请求路由到 `$xiaohuang-skill`；文章或长文改写交给 `$cs-writer`，实际 ChatCut 项目编辑和电商视频复刻继续使用各自的专用工作流。
